...
namespace AdaptiveCards.Rendering.MAUI
{
    ...
    {
        public static View Render(AdaptiveActionSet actionSet, AdaptiveRenderContext context)
        {
        ...
        public static void AddRenderedActions(Grid uiContainer, IList<AdaptiveAction> actions, AdaptiveRenderContext context,  AdaptiveInternalID actionSetId)
        {
            ...
            {
                StackBase uiActionBar;

                if (actionsConfig.ActionsOrientation == ActionsOrientation.Horizontal)
                    uiActionBar = new HorizontalStackLayout();
                else
                    uiActionBar = new VerticalStackLayout();

                switch (actionsConfig.ActionAlignment)
                {
                    case AdaptiveHorizontalAlignment.Left:
                        uiActionBar.HorizontalOptions = LayoutOptions.Start;
                        break;
                    ...
                    case AdaptiveHorizontalAlignment.Right:
                        uiActionBar.HorizontalOptions = LayoutOptions.End;
                        break;
                    case AdaptiveHorizontalAlignment.Stretch:
                        uiActionBar.HorizontalOptions = LayoutOptions.Fill;
                        break;
                }

                uiActionBar.VerticalOptions = LayoutOptions.End;
                ...
                foreach (AdaptiveAction action in actionsToProcess)
                {
                    ...
                    // add actions
                    var uiAction = context.Render(rendereableAction) as Border;
                    ...
                    if (action is AdaptiveShowCardAction showCardAction && isInline)
                    {
                        if (actionSetId != null)
                        {
                            ...
                            if (!hasSeenInlineShowCard)
                            {
                                ...
                                context.PeerShowCardsInActionSet[actionSetId] = new List<View>();
                            }
